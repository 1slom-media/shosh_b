...
class ChangeController {
    ...
    public async Post(req: Request, res: Response) {
        const { full_name,cash_coming,enum_coming,departure_date,staff,rooms,cash_flow,transfer_exp,comentary,consumption_category } = req.body

        const change = await AppDataSource.getRepository(ChangeEntity).createQueryBuilder().insert().into(ChangeEntity).values({full_name,cash_coming,enum_coming,departure_date,staff,rooms,cash_flow,transfer_exp,comentary,consumption_category}).returning("*").execute()
    ...
    public async Put(req: Request, res: Response) {
        try {
            ...
            const change = await AppDataSource.getRepository(ChangeEntity).createQueryBuilder().update(ChangeEntity)
                .set({full_name,cash_coming,enum_coming,departure_date,staff,rooms,cash_flow,transfer_exp,comentary,consumption_category})
                .where({ id })
                .returning("*")
                .execute()
