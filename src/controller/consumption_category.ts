...
class ConsumptionCategoryController {
    ...
    public async Post(req: Request, res: Response) {
        try {
            const { consumption_name } = req.body

            const consumption_category = await AppDataSource.getRepository(ConsumptionCategoryEntity).createQueryBuilder().insert().into(ConsumptionCategoryEntity).values({ consumption_name }).returning("*").execute()
